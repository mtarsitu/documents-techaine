...
namespace API_auto.mappers
{
    public static class AutoMapper
    {
        public static AutoId GetAutoData(AnalyzeResult result, double price, string plateNumber)
        {
            ...
            autoId.plateNumber = new PlateNumber(plateNumber != null ? plateNumber : "-", 0.8f);
            autoId.itpExpire = new ItpExpire("-", 0.8f);
            autoId.buyedAt = new BuyedAt("-", 0.8f);
            autoId.buyedWith = new BuyedWith("-", 0.8f);
            return autoId;
        }
