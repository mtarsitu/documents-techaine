...
function CompletedDoc({
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const response = await axios.post(`${backUrl}Email/sendPdfEmail`, formData);
  ...
  useEffect(() => {
    ...
    GetContractCompleted(seller, buyer, auto, signatures, coin).then((result) => {
      ...
      ToBase64(waterMarked ? result.watermarked : result.final).then((response) => {
    ...
    <>
      ...
      {step === 3 ? (
        <MKBox bgColor="white">
          ...
          {!paymentPage && (
            ...
            >
              ...
              <MKTypography variant="h6" mb={4}>
                Acesta este contractul tau de vanzare-cumparare. Speram ca totul este ok, in cazul
                in care ceva nu este in regula te rugam sa faci o poza si sa ne contactezi!
              </MKTypography>
          ...
          {paymentPage && (
            <Payment setWaterMarked={setWaterMarked} setPaymentPage={setPaymentPage} />
          )}
        </MKBox>
      ) : (
        <EditData
